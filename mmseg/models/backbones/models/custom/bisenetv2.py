...
class BGALayer(BaseModule):
    ...
    def forward(self, x_d, x_s, x_my):
        ...
        semantic_dwconv = self.semantic_dwconv(x_my) 
...
class BiSeNetV2(BaseModule):
    ...
    def forward(self, x):
        ...
        x_cat = torch.cat([x_semantic_lst_2[-1], x_semantic_lst_1[-1]], dim=1)      # (N, 2C, H, W)
        x_semantic_lst = self.fuser(x_cat) 
        #x_semantic_lst=x_semantic_lst_1[-1]*x_semantic_lst_1[-1]
        
        #x_head = self.bga(x_detail, x_semantic_lst[-1]) #my comment
        x_head = self.bga(x_detail, x_semantic_lst_2[-1], x_semantic_lst)
        outs = [x_head] + x_semantic_lst_2[:-1]
        #""" latest

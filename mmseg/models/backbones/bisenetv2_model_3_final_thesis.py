...
class BGALayer(BaseModule):
    ...
    def __init__(self,
        ...
        """
        #self.concat=FuseConcatSE(c=128, out_c=128)
        ...
        self.mix_conv = nn.Sequential(
            nn.Conv2d(
                in_channels=256,
                out_channels=128,   # reduce channels
                kernel_size=3,
                padding=1,
                bias=False
            ),
            ...
            nn.Conv2d(
                in_channels=128,
                out_channels=128,   # keep channels
                kernel_size=3,
                padding=1,
                bias=False
            ),
    ...
    def forward(self, x_d, x_s, x_my):
        ...
        fuse_2 = semantic_dwconv*torch.sigmoid(rgb_dwconv) #my comment
        semantic_conv = self.semantic_conv(fuse_2)
        semantic_conv = resize(
            input=semantic_conv,
            size=detail_dwconv.shape[2:],
            mode='bilinear',
        ...
        ##output=self.concat(fuse_1,fuse_2)
        #output=self.concat(fuse_1,fuse_2)
        return output
...
class BiSeNetV2(BaseModule):
    ...
    def forward(self, x):
        ...
        outs = [x_head] + x_semantic_lst_2[1:3]+x_semantic_lst_1[2:4] #conv_5

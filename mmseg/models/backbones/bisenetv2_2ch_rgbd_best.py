...
class BGALayer(BaseModule):
    ...
    def __init__(self,
        ...
        self.mix_conv = nn.Sequential(nn.Conv2d(in_channels=256, out_channels=128, kernel_size=3, padding=1, bias=False),nn.BatchNorm2d(128),nn.ReLU(inplace=True))
        self.out_channels = out_channels
        ...
        """
        self.semantic_dwconv_new = nn.Sequential(
            DepthwiseSeparableConvModule(
                in_channels=self.out_channels,
                out_channels=self.out_channels,
                kernel_size=3,
    ...
    def forward(self, x_d, x_s):
        ...
        #depth_dwconv = self.semantic_dwconv_new(x_my) 
        
        #dil = torch.nn.functional.max_pool2d(depth_dwconv, kernel_size=5, stride=1, padding=5 // 2)
        #depth_dwconv=depth_dwconv+self.thicker(dil)
        ...
        fuse_3 =  detail_down*torch.sigmoid(semantic_dwconv)
        ...
        x = torch.cat([fuse_1, fuse_3], dim=1)              # [B, C+C2, H, W]
...
class BiSeNetV2(BaseModule):
    ...
    def forward(self, x):
        ...
        ##x_semantic_lst_2 = self.semantic_new(x_semantic_input_rgb)

...
"""
class SemanticBranch_new(BaseModule):
    def __init__(self,
                 semantic_channels=(16, 32, 64, 128),
                 in_channels=3,
                 exp_ratio=6,
    ...
    def forward(self, x, other_stage1_16=None):
        
        #other_stage1_16: feature map from the OTHER semantic branch after its 16-ch stage.
        #                 Shape must match this branch's stage1 output: [B, 16, H, W]
        
        semantic_outs = []

        for idx, stage_name in enumerate(self.semantic_stages):
            semantic_stage = getattr(self, stage_name)
            x = semantic_stage(x)

            # ✅ After Stage-1 (16 channels), multiply with other branch's Stage-1 output
            if idx == 0 and other_stage1_16 is not None:
                # Safety checks
                assert x.shape == other_stage1_16.shape, \
                    f"Shape mismatch: x {x.shape} vs other_stage1_16 {other_stage1_16.shape}"
                x = x * other_stage1_16  # element-wise multiply
...
class BGALayer(BaseModule):
    ...
    def __init__(self,
        ...
        """
        """
        self.detail_down_rgb = nn.Sequential(
            ConvModule(
                in_channels=self.out_channels,
                out_channels=self.out_channels,
                kernel_size=3,
        ...
        self.mix_conv = nn.Sequential(
            nn.Conv2d(
                in_channels=384,   # input feature maps
    ...
    def forward(self, x_d, x_s, x_my):
        ...
        #rgb_detail= self.detail_down_rgb(x_my) 
...
class BiSeNetV2(BaseModule):
    ...
    def __init__(self,
        ...
        """
        self.detail_rgb = DetailBranch(self.detail_channels, self.in_channels//2)
        """
    ...
    def forward(self, x):
        ...
        outs = [x_head] + x_semantic_lst_1[:-1]#+ x_semantic_lst_2[1:3]  #+x_semantic_lst_1[1:4] #conv_5


        ##x_head = self.bga(x_detail, x_semantic_lst_1[-1]) #my comment
        ###x_head = self.bga(x_detail, x_semantic_lst_1[-1], x_semantic_lst_2[-1])
        ###outs = [x_head] + x_semantic_lst_1[2:4]+x_semantic_lst_2[2:4] #conv_5
        #outs = [x_head] + x_semantic_lst_2[:-1]#+x_semantic_lst_1[:-1]

...
model = dict(
    ...
    backbone=dict(
        ...
        align_corners=False),
decode_head=dict(
    type='DepthAwareFCNHead',   # <- changed
    in_channels=64,
    in_index=0,
    channels=1024,
    num_convs=1,
    ...
    align_corners=False,
    loss_decode=dict(
        type='DepthAwareCrossEntropyLoss',  # <- changed
        log_max_depth=True,
        log_interval=100, # print every 20 forward calls
        use_sigmoid=False,
        loss_weight=5,
        avg_non_ignore=True,                # recommended
        # ignore_index=255,                 # set if your dataset uses 255 (optional)

        # reduce loss for far pixels (example piecewise in meters)
        depth_cfg=dict(
                mode='linear',
                d_max=1,
...
auxiliary_head = [
    dict(
        ...
        loss_decode=dict(
            ...
            loss_weight=2,
            avg_non_ignore=True,          # recommended
            # ignore_index=255,            # uncomment if your dataset uses 255

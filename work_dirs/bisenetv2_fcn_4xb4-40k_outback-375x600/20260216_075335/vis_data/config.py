...
model = dict(
    ...
    decode_head=dict(
        ...
        in_index=0,
        loss_decode=dict(
            loss_weight=5.0, type='CrossEntropyLoss', use_sigmoid=False),
        norm_cfg=dict(requires_grad=True, type='SyncBN'),
        num_classes=5,
        num_convs=1,
        type='FCNHead'),
...
train_dataloader = dict(
    batch_size=8,
    dataset=dict(
        ann_file='outback_DICTA_NW/train_elevation_thesis.txt',
        data_prefix=dict(
            img_path='outback_DICTA_NW/rgbd_new',
            seg_map_path='outback_thesis/masks_3_levels_outback_thesis_depth'),
        data_root='../GANav-offroad/data/',
        pipeline=[
            dict(type='LoadNpyAsImage'),
            dict(type='LoadAnnotations'),
        ...
        seg_map_path_aux='outback_thesis/masks_3_levels_outback_thesis',
        type='OUTBACKDataset'),
    num_workers=4,
    persistent_workers=True,
...
val_dataloader = dict(
    ...
    dataset=dict(
        ...
        seg_map_path_aux='outback_thesis/test_masks_3_levels_thesis',
        type='OUTBACKDataset'),
    num_workers=4,
    persistent_workers=True,
    sampler=dict(shuffle=False, type='DefaultSampler'))
